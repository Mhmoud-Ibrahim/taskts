...
export const googleAuthSuccess = catchError(async (req, res) => {
    if (req.user) {
        sendTokenResponse(req.user, res);
        res.redirect('https://tasksnextjs.vercel.app');
    }
...
export const forgotPassword = catchError(async (req, res, next) => {
    const { email } = req.body;
    if (!email)
        return next(new AppError('برجاء إدخال البريد الإلكتروني', 400));
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    const hashedOtp = crypto.createHash('sha256').update(otp).digest('hex');
    const expires = new Date(Date.now() + 10 * 60 * 1000); // صالح لمدة 10 دقائق
    const user = await User.findOneAndUpdate({ email }, { passwordResetToken: hashedOtp, passwordResetExpires: expires }, { new: true });
    if (!user)
        return next(new AppError('لا يوجد مستخدم بهذا الإيميل', 404));
    try {
        await sendEmail({
            email: user.email,
            subject: 'رمز التحقق الخاص بك (OTP)',
